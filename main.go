...
func main() {
	...
	// 	reate a context that can be used to cancel goroutine
		...
		for {
			...
			case event := <-socketClient.Events:
				...
				case socketmode.EventTypeEventsAPI:
					...
					if err != nil {
						//replace wtih actual error handling
						log.Fatal(err)
					}
					//Handle slash commands
				case socketmode.EventTypeSlashCommand:
					//Type cast to correct event type, this time a SlashEvent
					command, ok := event.Data.(slack.SlashCommand)
					if !ok {
						log.Printf("Could not ype cast the message to a SlashCommand: %v\n", command)
						continue
					}
...
func handleSlashCommand(command slack.SlashCommand, client *slack.Client) (interface{}, error) {
	...
	case "/restart":
		// This was a restart command, so pass it along to the proper function
		return handleRestartCommand(command, client)
	case "/delete":
		return handleDeleteCommand(command, client)
...
// handleRestartCommand will take care of /restart submissions
func handleRestartCommand(command slack.SlashCommand, client *slack.Client) error {
	...
	// Greet the user
	attachment.Text = fmt.Sprintf("Restarting %s", command.Text)
	attachment.Color = "#4af030"
...
func handleDeleteCommand(command slack.SlashCommand, client *slack.Client) (interface{}, error) {
	// Create the attachment and assigned based on the message
	attachment := slack.Attachment{}

	// Create the checkbox element
	checkbox := slack.NewCheckboxGroupsBlockElement("answer",
		slack.NewOptionBlockObject("yes", &slack.TextBlockObject{Text: "Yes", Type: slack.MarkdownType}, &slack.TextBlockObject{Text: "Did you want to delete this grader?", Type: slack.MarkdownType}),
		slack.NewOptionBlockObject("no", &slack.TextBlockObject{Text: "No", Type: slack.MarkdownType}, &slack.TextBlockObject{Text: "Did you not want to delete this grader?", Type: slack.MarkdownType}),
	)
		...
		BlockSet: []slack.Block{
			...
			slack.NewSectionBlock(
				&slack.TextBlockObject{
					...
					Text: "Would you like to delete the grader?",
				},
				nil,
				accessory,
			),
	...
	attachment.Text = "Confirm this Deletion"
	attachment.Color = "#4af030"
